# ============================================================
#  training/train_mouth_model.py
#  Huấn luyện MobileNetV2 – Mouth: No_Yawn (0) / Yawn (1)
#
#  Dataset:  dataset/train/   dataset/valid/   dataset/test/
#            └─ no_yawn/         └─ no_yawn/      └─ no_yawn/
#            └─ yawn/            └─ yawn/          └─ yawn/
#
#  Chạy: python training/train_mouth_model.py
# ============================================================
...
from config import (
    ...
    MOUTH_MODEL_PATH, MODELS_DIR,
    IMG_SIZE, BATCH_SIZE, EPOCHS, LEARNING_RATE,
)

CLASSES = ["no_yawn", "yawn"]   # no_yawn=0, yawn=1
...
def train():
    ...
    print("\n=== [Mouth] Phase 1 – Head only (base frozen) ===")
    h1 = model.fit(
    ...
    print("\n=== [Mouth] Phase 2 – Fine-tune (30 layers cuối) ===")
    h2 = model.fit(
        train_gen, epochs=EPOCHS, validation_data=val_gen,
        callbacks=[
            ModelCheckpoint(MOUTH_MODEL_PATH, monitor="val_accuracy",
    ...
    print("\n=== [Mouth] Đánh giá trên TEST SET (held-out) ===")
    evaluate(model, test_gen)
    plot_history(h1, h2)
    print(f"\n✓ Model lưu tại: {MOUTH_MODEL_PATH}")
...
def evaluate(model, test_gen):
    ...
    print("\n" + classification_report(y_true, y_pred, target_names=["No_Yawn", "Yawn"]))

    cm = confusion_matrix(y_true, y_pred)
    plt.figure(figsize=(5, 4))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Oranges",
                xticklabels=["No_Yawn", "Yawn"], yticklabels=["No_Yawn", "Yawn"])
    plt.title("Confusion Matrix – Mouth Model (Test Set)")
    plt.ylabel("Actual"); plt.xlabel("Predicted")
    plt.tight_layout()
    path = os.path.join(MODELS_DIR, "mouth_confusion_matrix.png")
...
def plot_history(h1, h2):
    ...
    for ax, y_tr, y_v, title in [
        ...
        ax.set_title(f"Mouth Model – {title}"); ax.legend()

    plt.tight_layout()
    path = os.path.join(MODELS_DIR, "mouth_training_curves.png")
    plt.savefig(path); plt.close()
